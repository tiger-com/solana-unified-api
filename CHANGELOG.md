...
All notable changes to `@tigercom/perp-sdk` are recorded here. The format follows
...
- Realtime events at `@tigercom/perp-sdk/realtime`, deduplicating executions by
  `source_key` and position events by `event_id` in a bounded window.
- Transaction helpers at `@tigercom/perp-sdk/solana`: `submitAndConfirm`,
